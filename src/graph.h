...
/* Force-directed layout of the vault's wiki-link graph. Pure C, no SDL — the
 * model holds node positions in an arbitrary world space centered on the
 * origin; the renderer fits that to a viewport. One node per note, one edge
 * per `[[wiki link]]` between two notes. */

typedef struct {
    float x, y;          /* world-space position (origin-centered)    */
    float dx, dy;        /* scratch displacement, per layout step     */
    int   vault_idx;     /* index into the owning Vault.items array    */
...
/* Append a node for the given vault item. Returns its node index. */
